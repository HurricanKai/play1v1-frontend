...
socket.onOpen(() => {
  ...
  arena.on("full_state", (payload) => {
    ...
    gameState = {
        players: players.map(e => ({x: e.x, y: e.y, h: e.h, c: e.color, i: e.impact_counter })),
...
function flushGameState() {
    let players = gameState.players.map(({x, y, h, c}) => `<g
            xmlns="http://www.w3.org/2000/svg"
            transform="matrix(${Math.cos(h)} ${-Math.sin(h)} ${Math.sin(h)} ${Math.cos(h)} ${x} ${y})"
            >
                <path
    ...
    let bullets = gameState.bullets.map(({x, y, h}) => `<g
            xmlns="http://www.w3.org/2000/svg"
            transform="matrix(${Math.cos(h)} ${-Math.sin(h)} ${Math.sin(h)} ${Math.cos(h)} ${x} ${y})"
            >
            <path
    ...
    let impacts = gameState.impacts.map(({x, y}) => `<g
            xmlns="http://www.w3.org/2000/svg"
    ...
    let impact_counter = gameState.players.map(({c, i}, idx) => `
                <text x="3" y="${idx * 10 + 10}" style="fill: ${c}; font-size: 0.5rem">${i}</text>`)
            .join();

    self.postMessage({type: "render-update", payload: {players, bullets, impacts, impact_counter}});
}